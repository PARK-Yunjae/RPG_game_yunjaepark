...
public class GuildDAO {
	...
	private void BuyUnit(Player p) {
		...
		String[] n1 = { "박", "이", "김", "최", "유", "지", "오", "윤", "박", "손" };
		String[] n2 = { "명", "기", "종", "민", "재", "석", "광", "경", "연", "지" };
		String[] n3 = { "수", "자", "민", "수", "석", "민", "철", "재", "미", "영" };
	...
	private void NameSort() {
		for(int i=0 ; i<unitList.size() ; i+=1) {
			for(int k=i ; k<unitList.size() ; k+=1) {
				if(unitList.get(i).getName().charAt(0) > 
						unitList.get(k).getName().charAt(0)) {
	...
	private void AttackSort() {
		for(int i=0 ; i<unitList.size() ; i+=1) {
			for(int k=i ; k<unitList.size() ; k+=1) {
				if(unitList.get(i).getAtt() < 
						unitList.get(k).getAtt()) {
					Unit temp = unitList.get(i);
					unitList.set(i, unitList.get(k));
					unitList.set(k, temp);
				}
