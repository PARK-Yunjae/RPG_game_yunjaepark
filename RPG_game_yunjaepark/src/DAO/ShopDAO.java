...
public class ShopDAO {
	...
	private int SelectShopMenu(int sel, Player p) {
		...
		if(select != -1) {
			Item add = new Item(itemList.get(itemNum.get(select)).getKind(), itemList.get(itemNum.get(select)).getName()
					, itemList.get(itemNum.get(select)).getPower(), itemList.get(itemNum.get(select)).getPrice());
